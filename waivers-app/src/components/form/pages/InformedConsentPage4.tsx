...
export default function InformedConsentPage4({ formData, onInputChange }: InformedConsentPageProps) {
  const { template } = useTemplateByType('representative');

  if (!template || template.blocks.length < 5) {
    return <div className="text-red-600">Template not configured properly</div>;
  }

  const clauseBlock = template.blocks[4];
